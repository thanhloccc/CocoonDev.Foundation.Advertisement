...
namespace CocoonDev.Foundation.Advertisement
{
    public class AdDummyHandler : AdProviderHandler
    {
        ...
        public override void OnInitialize(AdsSettings settings)
        {
            ...
            {
                string path = "[AdsDummyCanvas]";
                GameObject dummyCanvasPrefab = Resources.Load<AdDummyController>(path).gameObject;
                if (dummyCanvasPrefab != null)
                {
                    GameObject dummyCanvas = Object.Instantiate(dummyCanvasPrefab);
